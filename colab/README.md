...
2. **Paste this into a cell and run it:**
   ...
   The script checks the GPU, clones this repo, runs `make test-gpu
   ARCH=sm_75` (aborting if any op disagrees with the CPU reference — **check
   that the printed table is all PASS**), then `make bench-gpu ARCH=sm_75`,
   and finally prints the CSV it wrote.
...
4. **Publish the numbers.** In your local clone of this repo:
   ...
   git add -A && git commit -m "results: t4 benchmark run" && git push
   ```
...
* A different GPU runtime (L4, A100) works too: the Makefile's default fat
  binary covers sm_75/sm_86 plus PTX for anything newer, so drop the
  `ARCH=sm_75` override or set it to the right arch, and the CSV/README will
  be labeled with the actual device name.
